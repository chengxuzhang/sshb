...
$(function(){(
	function(){
		...
		setTips=function(c,d){
			...
			var b=d?'value="'+d+'"':"";
			var model = a.data("model"),field = a.data("field"),id = a.data("id");
			var input = '<input type="hidden" name="'+model+'['+field+'][]" value="'+tid+'">';
			a.append($("<a "+b+' title="'+c+'" href="javascript:void(0);" ><span>'+c+"</span><em></em>"+input+"</a>"));
			searchAjax(c,d,true);
			return true
		};
...
// 搜索
(function(){
	var $b = $('.plus-tag-add button#addTag'),$i = $('.plus-tag-add input');
	// 键盘监听
	// $i.keyup(function(e){
	// 	if(e.keyCode == 13){
	// 		$b.click();
