<?php

use yii\helpers\Html;
use yii\widgets\ActiveForm;
?>

<div class="kline-up-shares-form">

    <?php $form = ActiveForm::begin([
        'options'=>['enctype'=>'multipart/form-data','class' => 'layui-form'],
        'fieldConfig' => [
            'template' => "<div class='layui-form-item'>{label}\n<div class=\"col-lg-4\">{input}</div>\n<div class=\"col-lg-6\">{error}</div></div>",
            'labelOptions' => ['class' => 'col-lg-2 control-label'],  //修改label的样式
        ],
    ]); ?>

    <?= $form->field($model, 'scode')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'sname')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'create_time')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'is_thirty_up')->textInput() ?>

    <?= $form->field($model, 'is_sun')->textInput() ?>

    <?= $form->field($model, 'is_one_down')->textInput() ?>

    <?= $form->field($model, 'is_more_head')->textInput() ?>

    <?= $form->field($model, 'is_five_up')->textInput() ?>

    <?= $form->field($model, 'is_two_down')->textInput() ?>

    <div class="form-group">
        <?= Html::button($model->isNewRecord ? 'Create' : 'Update', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary',"lay-submit"=>"", "lay-filter"=>"mainform"]) ?>
    </div>

    <?php ActiveForm::end(); ?>

</div>

<script>
    layui.use(['form'], function(){
        var form = layui.form;

        //监听提交
        form.on('submit(mainform)', function(data){
            $.post("<?= Yii::->url; ?>",data.field,function (res) {
                if(res.status == 200){
                    layer.msg(res.msg,function () {
                        window.location.href = "/kline-up-shares";
                    });
                }else{
                    layer.msg(res.msg);
                }
            });
            return false;
        });
    });
</script>
