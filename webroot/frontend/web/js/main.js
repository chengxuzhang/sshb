...
$(function(){
    $('#wrap').fullpage({
        ...
        afterLoad: function(anchorLink, index){
            // $('video').get(0).play();
//				if(index == '1'){
//					$('.m_loser').css('display','none');
//				}
        },
        onLeave: function(index, direction){

//			if(index == '1'){
//				$('.m_loser').css('display','block');
//			}
        }
    });
});
...
$(function(){
    $('.zj_bvmenu').hover(function(){
        $(this).find(".zj_bmenu").css('display','block');
    },function(){
        $(this).find(".zj_bmenu").css('display','none');
    })
})
...
$(function () {
    function runAuto() {
        $(".auto-height").each(function (key, item) {
            var scale = $(item).data("scale");
            var scale_w = parseInt(scale.split("|")[0]);
            var scale_h = parseInt(scale.split("|")[1]);
            var width = $(window).width();
            var height = (width * scale_h) / scale_w;
            $(item).css("height", height + "px");
        })
    }
    window.addEventListener( 'resize', runAuto, false );
    runAuto();
})